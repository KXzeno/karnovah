...
export default async function Curiograph() {
  let feeds: Array<object> = [await parse(FEED_URLS[0]), await parse(FEED_URLS[1]), await parse(FEED_URLS[2])];

  return (
    <div className='relative min-h-screen w-screen'>
      <div className='flex-row text-center space-y-4'>
        <p>Begins here</p>
        {Object.values(feeds[2]).map(({ title, link, description, pubDate, guid }) => {
            return (
              <div key={pubDate}>
